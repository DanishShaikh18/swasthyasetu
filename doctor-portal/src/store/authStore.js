import { create } from 'zustand'
import { persist } from 'zustand/middleware'
const useAuthStore = create(persist((set) => ({
  accessToken: null, refreshToken: null, user: null, isAuthenticated: false,
  setAuth: (d) => set({ accessToken: d.access_token, refreshToken: d.refresh_token, user: { id: d.user_id, role: d.role, fullName: d.full_name, isApproved: d.is_approved }, isAuthenticated: true }),
  setToken: (t) => set({ accessToken: t }),
  logout: () => set({ accessToken: null, refreshToken: null, user: null, isAuthenticated: false }),
}), { name: 'doctor-auth' }))
export default useAuthStore
